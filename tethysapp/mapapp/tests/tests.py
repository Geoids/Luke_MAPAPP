    ...
    4. Enter tethys test command.
       ...
       See below for specific examples
        ...
        To run all tests across this app:
            Test command: "tethys test -f tethys_apps.tethysapp.mapapp"

        To run all tests in this file:
            Test command: "tethys test -f tethys_apps.tethysapp.mapapp.tests.tests"

        To run tests in the MapappTestCase class:
            Test command: "tethys test -f tethys_apps.tethysapp.mapapp.tests.tests.MapappTestCase"

        To run only the test_if_tethys_platform_is_great function in the MapappTestCase class:
            Test command: "tethys test -f tethys_apps.tethysapp.mapapp.tests.tests.MapappTestCase.test_if_tethys_platform_is_great"
...
class MapappTestCase(TethysTestCase):
    """
    ...
    def set_up(self):
        ...
        test database creation function here, like so:
            ...
            self.create_test_persistent_stores_for_app(Mapapp)
        ...
        get a test client, like so:
            ...
            self.user = self.create_test_user(username="joe", password="secret", email="geoidsbyu@gmail.com")
    ...
    def tear_down(self):
        ...
        stores, you should call the test database destruction function from here, like so:
            ...
            self.destroy_test_persistent_stores_for_app(Mapapp)
    ...
    def test_home_controller(self):
        ...
        user = self.create_test_user(username="joe", password="secret", email="geoidsbyu@gmail.com")
        c.force_login(user)

        # Have the test client "browse" to your home page
        response = c.get('/apps/mapapp/')  # The final '/' is essential for all pages/controllers
